using BlockSense.Client.Cryptography.Encryption;
using BlockSense.Client.Cryptography.Hashing;
using BlockSense.Client.Cryptography.KeyDerivation;
...
namespace BlockSense.Client.Cryptography.Wallet
{
    class MnemonicManager
    {
        ...
        private static readonly SecureLevelDbBinaryStore _db;

        static MnemonicManager()
        {
            _db = new SecureLevelDbBinaryStore(Path.Combine(DirStructure.walletPath, "leveldb"));
        }
        ...
        {
            byte[] salt = CryptoUtils.SecureRandomGenerator(SaltSize);
