...
public partial class InviteManagerWindow : Window
{
    ...
    private readonly UserInvites _invites;

    private Border? _currentExpandedBorder = null;

    public InviteManagerWindow(ApiClient apiclient, MainWindow mainWindow, UserInvites userInvites)
    {
        _apiClient = apiclient;
        _mainWindow = mainWindow;
        _invites = userInvites;
    ...
    {
        var fetchedInvites = await _apiClient.FetchInviteInfo() ?? new();

        // Clear the existing singleton list and add the fetched invites
        _invites.Invites.Clear();
        _invites.Invites.AddRange(fetchedInvites.Invites);

        DisplayInvites(_invites.Invites);
    }


    private void DisplayInvites(List<InviteInfo> invitesToDisplay)
    {
        ...
        {
            InviteInfo invite = invitesToDisplay[iRow];
    ...
    private void InviteFinder(object sender, TextChangedEventArgs e)
    {
        ...
        List<InviteInfo> filteredInviteCodes = new();
        ...
        foreach (InviteInfo invite in _invites.Invites)
        {
    ...
    public void ExpandInviteCode(object? sender, PointerPressedEventArgs e)
    {
        ...
        textBlock.Text = _invites.Invites[Grid.GetRow(clickedBorder)].InvitationCode;
